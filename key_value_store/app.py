...
REPLICA_ID = 0
SHARD_ID = 0 # CHANGE: shard ID's start at 0
...
# NOTE: Shards ID's start at 0
# BUT: accessing shards in the shards list start at 0
...
def keys(key):
    ...
    if not (MAIN_ADDR in SHARD_LIST[shard]):
        try:
            #TODO change response for failed proxy, indicating address proxied
            #TODO may need to resend request in case replica 0 is down
